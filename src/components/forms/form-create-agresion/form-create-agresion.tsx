...
export const formCreateAgresionSchema = z.object({
  fechaAgresion: z.date({
    required_error: "La fecha de la agresión es requerida",
  }),
  establecimientoId: z
    .number({ required_error: "El establecimiento es requerido" })
    .int()
    .min(1, { message: "Debes seleccionar un establecimiento" }),
  empleadoId: z
    .number({ required_error: "El empleado es requerido" })
    .int()
    .min(1, { message: "Debes seleccionar un empleado" }),
  tipoAgresionId: z
    .number({ required_error: "El tipo de agresión es requerido" })
    .int()
    .min(1, { message: "Debes seleccionar un tipo de agresión" }),
  categoriasAgresionesId: z
    .array(z.number())
  ...
  agresores: z
    .array(
      z.object({
        tipoAgresorId: z
          .number({ required_error: "El tipo de agresor es requerido" })
          .int()
          .min(1, { message: "Debes seleccionar un tipo de agresor" }),
        name: z.string().optional(),
        lastName: z.string().optional(),
        rut: z
          .string({ required_error: "Debes ingresar un rut" })
    ...
    .min(1, {
      message: "Se requiere al menos un agresor para registrar un incidente",
    }),
  testigos: z.array(
    z.object({
      name: z
        .string({ required_error: "Debes ingresar un nombre" })
        .min(1, "El nombre es requerido"),
      lastName: z
        .string({ required_error: "Debes ingresar un apellido" })
        .min(1, "El apellido es requerido"),
      rut: z
        .string({ required_error: "Debes ingresar un rut" })
        .min(1, "El RUT es requerido"),
      email: z
        .string({ required_error: "Debes ingresar un correo electrónico" })
...
const FormCreateAgresion = () => {
  ...
  const {
    fields: fieldsTestigos,
    append: appendTestigo,
    remove: removeTestigo,
  } = useFieldArray({
    control,
    name: "testigos",
  });
  ...
  useEffect(() => {
    if (data && establecimientoId !== 0) {
      setEmpleados(
        data?.empleados.filter(
          (empleado) => empleado.establecimientoId === establecimientoId
        ) || []
      );
    }
    ...
    if (data && tipoAgresionId != 0) {
      ...
      const categorias: LookupDto[] =
        data.tipoAgresionesCategorias
          .filter((categoria) => categoria.tipoAgresionId === tipoAgresionId)
          .map((categoria) => ({
            id: categoria.id,
            name: categoria.name,
          })) || [];
  ...
  const onSubmit = async (data: z.infer<typeof formCreateAgresionSchema>) => {
    let agresores: AgresorCreateDto[] = [];
    let testigos: TestigoCreateDto[] = [];

    if (data.agresores.length != 0) {
      agresores = data.agresores.map((item) => ({
        tipoAgresorId: item.tipoAgresorId,
        rut: item.rut,
        name:
          item.name === undefined || item.name.length === 0 ? null : item.name,
        lastName:
          item.lastName === undefined || item.lastName.length === 0
            ? null
            : item.lastName,
        address:
          item.address === undefined || item.address.length === 0
            ? null
            : item.address,
        comunaId:
          item.comunaId === 0 || item.comunaId === undefined
            ? null
            : item.comunaId,
      }));
    }

    if (data.testigos.length != 0) {
      testigos = data.testigos.map((item) => ({
        rut: item.rut,
        name: item.name,
            ...
            <Alert className="border border-green-600 bg-green-100">
              <div className="flex start">
                <p className="text-green-800 mr-2">
                  Se ha registrado la agresión correctamente, el afectado debe
                  revisar y aprobar esta agresión para finalizar el proceso.
                </p>{" "}
            ...
            <Alert className="border border-red-600 bg-red-100">
              <div className="flex start">
                <p className="text-red-800 mr-2">
                  Ocurrio un error al agregar el registro de agresión. Intente
                  nuevamente.
                </p>{" "}
                ...
                render={({ field }) => (
                  <FormItem className="w-full">
                    ...
                    <FormDescription className="flex gap-2">
                      <span>
                        Selecciona el establecimiento al cual pertenece el
                        empleado agredido
                      </span>
                    </FormDescription>
                  </FormItem>
                  ...
                  {empleados.length != 0 ? (
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="w-full">
                          ...
                          <FormDescription className="flex gap-2">
                            <span>
                              Selecciona el empleado que fue victima de una
                              agresión
                            </span>
                          </FormDescription>
                        </FormItem>
                    ...
                    <Alert className="flex items-center gap-1 border-red-500">
                      ...
                      <p className="text-red-500">
                        No hay empleados registrados para el establecimiento
                      </p>
                    </Alert>
                  )}
                </>
            ...
            <div className="my-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Selecciona la fecha y los tipos de agresiones de los cuales
                    ha sido víctima el afectado
                  </CardDescription>
                </CardHeader>
                <CardContent>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem className="flex flex-col">
                              ...
                              <FormDescription>
                                Selecciona la fecha del día que ocurrió la
                                agresión
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          ...
                          render={({ field }) => (
                            <FormItem className="w-full">
                              ...
                              >
                                <FormControl>
                                  <SelectTrigger>
                                    <SelectValue placeholder="Selecciona un tipo de agresión" />
                                  </SelectTrigger>
                                </FormControl>
                                <SelectContent>
                              ...
                              <FormDescription className="flex gap-2">
                                <span>
                                  Selecciona el tipo de agresión de la que ha
                                  sido víctima el afectado
                                </span>
                              </FormDescription>
                            </FormItem>
                          )}
                        ...
                        render={() => (
                          <FormItem>
                            <div className="mb-4">
                              ...
                              <FormDescription>
                                Selecciona uno o más tipos de agresión de los
                                cuales fue víctima el afectado
                              </FormDescription>
                            </div>
                            {categorias.map((item) => (
                              <FormField
                                  ...
                                  >
                                    <FormControl>
                                      <Checkbox
                                        checked={watch(
                                          "categoriasAgresionesId"
                                        ).includes(item.id)} // Use watch to keep track of the field value
            ...
            <div className="my-4">
              <Card>
                <CardHeader className="pb-0">
                  ...
                  <CardDescription>
                    Ingresa la información sobre el o los agresores involucrados
                    en el incidente. Es importante ser preciso y detallado al
                    completar está información, ya que facilitará un manejo
                    adecuado de la denuncia, y en caso de ser necesario ayudará
                    a las autoridades a tomar las medidas necesarias.
                  </CardDescription>
                  {errors.agresores && errors.agresores.message && (
                ...
                <CardContent>
                            ...
                            render={({ field }) => (
                              <FormItem className="w-full">
                                ...
                                >
                                  <FormControl>
                                    <SelectTrigger>
                                      <SelectValue placeholder="Selecciona un tipo de agresor" />
                                    </SelectTrigger>
                                  </FormControl>
                                  <SelectContent>
                  ...
                  <Button
                    ...
                    className="mt-4"
                  >
                    <span className="mr-1">Agregar nuevo agresor</span>{" "}
                    <UserRoundPlus />
                  </Button>
                </CardContent>
              </Card>
            ...
            <div className="my-4">
              <Card>
                <CardHeader className="pb-0">
                  ...
                  <CardDescription>
                    Ingresa la información sobre las personas que presenciaron
                    el incidente. Es fundamental asegurar que la información es
                    válida, ya que los testimonios pueden ser cruciales para
                ...
                <CardContent>
                  ...
                  <Button
                    ...
                    className="mt-4"
                  >
                    <span className="mr-1">Agregar nuevo testigo</span>{" "}
                    <UserRoundPlus />
                  </Button>
                </CardContent>
              </Card>
